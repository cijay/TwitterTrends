...
def analyze_tweet_sentiment(tweet):
    ...
    w, total, cpt = tweet_words(tweet), 0, 0
    for elem in w:
        if has_sentiment(get_word_sentiment(elem)):
            total, cpt = total + sentiment_value(get_word_sentiment(elem)), \
                    cpt + 1
    if cpt != 0:
        average = make_sentiment(total / cpt)
    else:
        average = make_sentiment(None)
    return average
...
def find_centroid(polygon):
    ...
    while i < len(polygon) - 1:
        b, b1, c, c1 = polygon[i][0], polygon[i+1][0], polygon[i][1], polygon[i+1][1]
        area += b*c1 - b1*c
        x += (b + b1)*(b*c1 - b1*c)
        y += (c + c1)*(b*c1 - b1*c)
        i += 1
...
def find_closest_state(tweet, state_centers):
    ...
    'NJ'
    """
    minimum, state = 0, None
    for elem in state_centers:
        dist = geo_distance(tweet_location(tweet), state_centers[elem])
        if minimum == 0:
            minimum = dist
            state = elem
        elif  dist < minimum:
...
def group_tweets_by_state(tweets):
    ...
    us_centers = {n: find_state_center(s) for n, s in us_states.items()}
    for elem in tweets:
        state = find_closest_state(elem, us_centers)
...
def most_talkative_states(term):
    ...
    """
    lst, cpt = group_tweets_by_state(tweets), 0
    for elem in lst:
        freq = len(lst[elem])
        if freq > cpt:
            cpt = freq
            most_talkative = elem
    return (most_talkative, cpt)
    """
    dic, lst = group_tweets_by_state(tweets), []
    for elem in group_tweets_by_state(tweets):
        freq = len(dic[elem])
        lst.append((elem,freq))
    lst = sorted(sorted(lst, key = lambda alpha: alpha[0]), key = lambda freq: \
            freq[1], reverse = True)
    return lst[:5]
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        sentiment, cpt = 0, 0
        for elem in tweets_by_state[state]:
            feeling = analyze_tweet_sentiment(elem)
            if has_sentiment(feeling):
                sentiment, cpt = sentiment + feeling, cpt + 1
            if cpt != 0:
                averaged_state_sentiments[state] = sentiment / cpt
...
def group_tweets_by_hour(tweets):
    ...
    tweets_by_hour = {}
    for elem in tweets:
        hour = tweet_time(elem).hour
        if hour not in tweets_by_hour:
            tweets_by_hour[hour] = [elem]
        else:
            tweets_by_hour[hour].append(elem)
